...
local storedESPColors = {} -- Store ESP colors for each egg
...
infoBtn.MouseButton1Click:Connect(function()
    ...
    infoLabel.Text = "🎲 Randomize eggs to change what's inside and see different possible pets!\n\n👁️ ESP displays pet information above eggs\n\n💡 Toggle ESP on/off to control visibility"
...
-- Show ESP
local function showPetESP(model, eggName, petText, textColor, strokeColor)
    ...
    espGui.Name = "PetESP"
    espGui.Size = UDim2.new(0, 150, 0, 30) -- Smaller height like original
    espGui.StudsOffset = Vector3.new(0, 3, 0)
    espGui.AlwaysOnTop = true

    -- Egg name label (always white)
    local eggLabel = Instance.new("TextLabel", espGui)
    eggLabel.Size = UDim2.new(1, 0, 0.5, 0)
    eggLabel.Position = UDim2.new(0, 0, 0, 0)
    eggLabel.BackgroundTransparency = 1
    eggLabel.Font = Enum.Font.GothamBold
    eggLabel.TextSize = 14 -- Increased text size
    eggLabel.TextColor3 = Color3.fromRGB(255, 255, 255) -- Always white
    eggLabel.Text = eggName
    eggLabel.TextXAlignment = Enum.TextXAlignment.Center
    eggLabel.TextYAlignment = Enum.TextYAlignment.Center
    eggLabel.TextStrokeTransparency = 0.5
    eggLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)

    -- Pet name label (changes color)
    local petLabel = Instance.new("TextLabel", espGui)
    petLabel.Size = UDim2.new(1, 0, 0.5, 0)
    petLabel.Position = UDim2.new(0, 0, 0.5, 0)
    petLabel.BackgroundTransparency = 1
    petLabel.Font = Enum.Font.GothamBold
    petLabel.TextSize = 14 -- Increased text size
    petLabel.TextColor3 = textColor or Color3.fromRGB(255, 255, 255)
    petLabel.Text = petText
    petLabel.TextXAlignment = Enum.TextXAlignment.Center
    petLabel.TextYAlignment = Enum.TextYAlignment.Center
    petLabel.TextStrokeTransparency = 0.5
    petLabel.TextStrokeColor3 = strokeColor or Color3.fromRGB(0, 0, 0)
    
    -- Store the colors for this egg
    storedESPColors[model.Name] = {
        textColor = petLabel.TextColor3,
        strokeColor = petLabel.TextStrokeColor3
    }
end

-- Check if egg is ready to hatch
local function isEggReadyToHatch(egg)
    -- Check if egg has a timer (not ready to hatch)
    local timer = egg:FindFirstChild("Timer") or egg:FindFirstChild("Countdown")
    if timer then
        return false
    end
    
    -- Check if egg is being held by player
    local owner = egg:FindFirstChild("Owner")
    if owner and owner.Value == player then
        return false
    end
    
    -- Check if egg has a "Ready" or "Hatchable" property
    local ready = egg:FindFirstChild("Ready") or egg:FindFirstChild("Hatchable")
    if ready and not ready.Value then
        return false
    end
    
    -- Check if egg has a "State" property that indicates it's not ready
    local state = egg:FindFirstChild("State")
    if state and (state.Value == "Incubating" or state.Value == "Growing" or state.Value == "Timer") then
        return false
    end
    
    -- If none of the above conditions are met, assume it's ready to hatch
    return true
end
...
local function getNearbyEggs(radius)
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and petChances[obj.Name] then
            -- Only include eggs that are ready to hatch
            if isEggReadyToHatch(obj) then
...
-- Helper function to get all possible pets for animation (including 1% chance pets)
local function getAllAnimationPets(eggName)
    local chances = petChances[eggName]
    if not chances then return {} end
    
    local allPets = {}
    
    -- Add all pets with >0 chance
    for pet, chance in pairs(chances) do
        if chance > 0 then
    ...
    -- Add animation-only pets
    local animationPet = animationOnlyPets[eggName]
    if animationPet then
        table.insert(allPets, animationPet)
    end
...
local function randomizeEggs(randomizeButton, espButton)
    ...
    for _, egg in ipairs(eggs) do
        ...
        if pets then
            ...
            if part and ESP_ENABLED then
                -- Clear any existing ESP first
                clearESP(egg)
                
                local success, espGui = pcall(function()
                    local gui = Instance.new("BillboardGui", part)
                    gui.Name = "PetESP"
                    gui.Size = UDim2.new(0, 150, 0, 30) -- Smaller height like original
                    gui.StudsOffset = Vector3.new(0, 3, 0)
                    gui.AlwaysOnTop = true
                    return gui
                end)
                
                if success and espGui then
                    local success2, eggLabel = pcall(function()
                        local label = Instance.new("TextLabel", espGui)
                        label.Size = UDim2.new(1, 0, 0.5, 0) -- Top half for egg name
                        label.Position = UDim2.new(0, 0, 0, 0)
                        label.BackgroundTransparency = 1
                        label.Font = Enum.Font.GothamBold
                        label.TextSize = 14 -- Increased text size
                        label.TextColor3 = Color3.fromRGB(255, 255, 255) -- Always white
                        label.TextXAlignment = Enum.TextXAlignment.Center
                        label.TextYAlignment = Enum.TextYAlignment.Center
                        label.TextStrokeTransparency = 0.5
                        label.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
                        label.Text = egg.Name -- Egg name always stays the same
                        return label
                    end)
                    
                    local success3, petLabel = pcall(function()
                        local label = Instance.new("TextLabel", espGui)
                        label.Size = UDim2.new(1, 0, 0.5, 0) -- Bottom half for pet name
                    ...
                    if success2 and eggLabel and success3 and petLabel then
    ...
    task.spawn(function()
        ...
        while tick() - startTime < 2 do
            for _, element in ipairs(espElements) do
                -- Check if element and labels still exist
                if element and element.petLabel and element.petLabel.Parent then
                    local allPets = getAllAnimationPets(element.egg.Name)
                    if #allPets > 0 then
                        -- Cycle through all pets rapidly
                        local currentIndex = math.floor((tick() - startTime) * 20) % #allPets + 1
                        local currentPet = allPets[currentIndex]
                        
                        element.petLabel.Text = "🎲 " .. currentPet .. " 🎲"
                        
                        -- Casino color effects - only color the pet line
                        local randomColor = colors[math.random(1, #colors)]
                        element.petLabel.TextColor3 = randomColor
                        element.petLabel.TextStrokeColor3 = randomColor -- Match the text color
                        element.petLabel.TextStrokeTransparency = 0.2 -- More visible stroke
                        
                        -- Scale animation for pet label only
                        local scale = 1 + (math.sin(tick() * 10) * 0.1)
                        element.petLabel.Size = UDim2.new(scale, 0, 0.5, 0)
                        element.petLabel.Position = UDim2.new((1 - scale) / 2, 0, 0.5, 0)
                    end
                end
            end
            task.wait(0.05) -- Even faster animation to show all pets clearly
        end
        ...
        for _, element in ipairs(espElements) do
            ...
            if element and element.petLabel and element.petLabel.Parent then
                ...
                if finalPet then
                    ...
                    element.petLabel.Text = "🎉 " .. finalPet .. " 🎉"
                    
                    -- Final celebration effect
                    local goldColor = Color3.fromRGB(255, 215, 0) -- Gold
                    element.petLabel.TextColor3 = goldColor
                    element.petLabel.TextStrokeColor3 = goldColor -- Match the text color
                    element.petLabel.TextStrokeTransparency = 0.2
                    element.petLabel.Size = UDim2.new(1, 0, 0.5, 0)
                    element.petLabel.Position = UDim2.new(0, 0, 0.5, 0)
                    
                    -- Store the final celebration colors
                    storedESPColors[element.egg.Name] = {
                        textColor = goldColor, -- Gold
                        strokeColor = goldColor -- Match the text color
                    }
                end
            end
        end
        
        -- Reset randomization state after 2 seconds
        isRandomizing = false
        
        -- Re-enable buttons after animation completes
        if randomizeButton and randomizeButton.Parent then
            randomizeButton.Text = "🎲 RANDOMIZE"
            randomizeButton.BackgroundColor3 = Color3.fromRGB(85, 255, 105) -- Back to original green
            randomizeButton.Active = true -- Re-enable the button
        end
        
        if espButton and espButton.Parent then
            espButton.Active = true -- Re-enable ESP button
            espButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60) -- Back to original color
        end
    end)
end
...
espBtn.MouseButton1Click:Connect(function()
    if isRandomizing then return end -- Disable during randomization
    ...
    for _, egg in ipairs(getNearbyEggs()) do
        ...
        if ESP_ENABLED and lastRandomizedPets[egg.Name] then
            -- Use the last randomized pet value and restore stored colors
            local storedColors = storedESPColors[egg.Name]
            local textColor = storedColors and storedColors.textColor or Color3.fromRGB(255, 215, 0)
            local strokeColor = storedColors and storedColors.strokeColor or textColor -- Match the text color
            showPetESP(egg, egg.Name, "🎉 " .. lastRandomizedPets[egg.Name] .. " 🎉", textColor, strokeColor)
        end
    end
end)
...
randomBtn.MouseButton1Click:Connect(function()
    ...
    -- Disable both buttons during randomization
    randomBtn.Text = "🎲 RANDOMIZING..."
    randomBtn.BackgroundColor3 = Color3.fromRGB(255, 165, 0) -- Orange during randomization
    randomBtn.Active = false -- Disable the button
    
    espBtn.Active = false -- Disable ESP button
    espBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100) -- Gray out ESP button
    
    randomizeEggs(randomBtn, espBtn)
end)
    ...
    if ESP_ENABLED then
        for _, egg in ipairs(getNearbyEggs()) do
            ...
            if pet then
                ...
                showPetESP(egg, egg.Name, "🎲 " .. pet .. " 🎲", Color3.fromRGB(255, 255, 255), Color3.fromRGB(0, 0, 0))
            end
        end
    end
end)
