    ...
    connection = RunService.Heartbeat:Connect(function()
        ...
        if progress >= 1 then
            ...
            for _, element in ipairs(espElements) do
                local finalPet = element.pets[math.random(1, #element.pets)]
                lastRandomizedPets[element.egg.Name] = finalPet
                element.label.Text = element.egg.Name .. " → " .. finalPet
            end
            connection:Disconnect()
            isRandomizing = false
            
            -- Reset button text and make it clickable again
            randomBtn.Text = "🎲 RANDOMIZE"
            randomBtn.BackgroundColor3 = Color3.fromRGB(85, 255, 105)
            return
        end
        
        -- Randomize text during animation
        for _, element in ipairs(espElements) do
            local randomPet = element.pets[math.random(1, #element.pets)]
            element.label.Text = element.egg.Name .. " → " .. randomPet
        end
        
        task.wait(0.1) -- 0.1ms interval
    end)
end
...
randomBtn.MouseButton1Click:Connect(function()
    ...
    -- Change button to show randomizing state
    randomBtn.Text = "🎲 RANDOMIZING..."
    randomBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
